...
export default function OutstandingUserCard(
    { userDigest } : { userDigest: { id: number, username: string, nickname: string, balance: number, enteredAt: Date, leftAt: Date | null, charge: number, chargeMultiplier: number }}
) {
    return (
        <Link href={`/manage/users/${userDigest.id}`} className="windowlike master-width">
            <div className="bipartite">
                <span><UserOneline user={{...userDigest, role: null, chargeMultiplier: userDigest.chargeMultiplier }} /></span>
