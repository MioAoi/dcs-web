...
export function calculateCharge (
    ...
    while (true) {
        ...
        if (isSameShanghaiDay(cursorDate, leaveTime) && leaveMinute <= currentSegment.endMinute) {
            ...
            total += duration * currentSegment.rate * (currentSegment.globalDiscountApply ? GLOBAL_DISCOUNT : 1);
            priceDetail += formatTimeMinutes(cursorMinute) + " 至 " + formatTimeMinutes(leaveMinute) + " 共 " + formatMoneyFen(duration * currentSegment.rate * (currentSegment.globalDiscountApply ? GLOBAL_DISCOUNT : 1)) + "\n";
            break;
        } else {
            const nextMinute = currentSegment.endMinute;
            const duration = nextMinute - cursorMinute;
            total += duration * currentSegment.rate * (currentSegment.globalDiscountApply ? GLOBAL_DISCOUNT : 1);
            priceDetail += formatTimeMinutes(cursorMinute) + " 至 " + formatTimeMinutes(nextMinute) + " 共 " + formatMoneyFen(duration * currentSegment.rate * (currentSegment.globalDiscountApply ? GLOBAL_DISCOUNT : 1)) + "\n";
